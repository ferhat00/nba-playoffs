"""
End-to-end orchestrator for the 2025 NBA Playoff Prediction System.
...
def run(n_simulations: int, mcmc_samples: int) -> None:
    ...
    log.info("Step 2/6: fitting BayesianPlayerEvaluator (MCMC samples=%d)", mcmc_samples)
    if mcmc_samples < 500:
    ...
    log.info("Step 3/6: building TeamStateAggregator")
    ...
    # 5. Simulate.
    log.info("Step 5/6: simulating %d brackets", n_simulations)
    ...
    print("\n=== 2025 NBA Playoff — Championship Probabilities ===")
    pd.set_option("display.float_format", lambda v: f"{v:.2f}")
    print(table.to_string(index=False))
    print()

    out_path = simulator.visualize()
    log.info("Saved bracket visualization: %s", out_path)


def main() -> None:
    parser = argparse.ArgumentParser(description="2025 NBA Playoff Prediction System")
    parser.add_argument(
        "--quick",
        action="store_true",
